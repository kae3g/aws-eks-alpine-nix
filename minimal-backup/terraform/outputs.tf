...
output "estimated_monthly_cost" {
  ...
  value = {
    ...
    total       = "~$${72 + (var.node_group_desired_size * 30) + (length(var.public_subnet_cidrs) * 45)}/month"
  }
}
...
output "connection_info" {
  ...
  value = {
    ...
    kubectl_config_command = "aws eks update-kubeconfig --region ${var.aws_region} --name ${aws_eks_cluster.main.name}"
  }
}
